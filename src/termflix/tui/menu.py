...
def _draw_menu(
    ...
    *,
    ...
    """Draw a centered menu — full clear on first draw, cursor-home on updates."""
    cols = _cols()
    lines = _lines()
    buf: list[str] = []

    buf.append(CLEAR_SCREEN if first_draw else CURSOR_HOME)
...
def run_home() -> str | None:
    ...
    try:
        while True:
            _draw_menu("termflix", items, sel, first_draw=first)
            first = False
            key = _get_key()
...
def run_browse(start: Path | None = None) -> Path | None:
    """Folder browser. Returns None when user exits."""
    ...
    try:
        while True:
            ...
            while True:
                buf = [CLEAR_SCREEN if first else CURSOR_HOME]
                first = False
                ...
                # clear any leftover lines below visible items
                for r in range(len(visible), max_visible):
                    buf.append(_move(3 + r, 0) + "\033[2K")
                ...
                if key == "up":
                    ...
                    if kind in ("dir", "up"):
                        ...
                        sys.stdout.write(HIDE_CUR)
                        first = True  # redraw browse cleanly on return
                    elif kind == "video":
    ...
    try:
        while True:
            ...
            if error:
                ...
                buf.append(_move(error_row, 0) + "\033[2K")
